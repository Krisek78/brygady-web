...
async function selectProject(id) {
    ...
    try {
        ...
        teams.forEach(team => {
            ...
            // ZMODYFIKOWANA SEKCJA LUDZI - dodano przycisk edycji ✎ oraz usuwanie trwałe 🗑
            const mHtml = team.members?.length ? 
                team.members.map(m => `
                    <div class="team-member-item" draggable="true" data-id="${m.id}" data-type="worker" data-team-id="${team.id}">
                        ...
                        <div style="display:flex; gap:4px;">
                            ...
                            <button class="iconBtn" style="color:var(--danger);" onclick="event.stopPropagation(); deleteWorker(${m.id})" title="Usuń trwale z bazy">🗑</button>
                        </div>
                    </div>
                `).join('') : 
                '<div style="color:#64748b; font-size:12px; padding:8px;">Brak ludzi</div>';
            
            // ZMODYFIKOWANA SEKCJA ZADAŃ - dodano przycisk edycji ✎ oraz usuwanie trwałe 🗑
            const tHtml = team.tasks?.length ? 
                team.tasks.map(t => `
                    <div class="team-task-item" draggable="true" data-id="${t.id}" data-type="task" data-team-id="${team.id}">
                        <span>${escapeHtml(t.title)}</span>
                        <div style="display:flex; gap:4px;">
                            <button class="iconBtn" onclick="event.stopPropagation(); editTask(${t.id}, '${escapeHtml(t.title)}')" title="Edytuj zadanie">✎</button>
                            <button class="iconBtn" onclick="event.stopPropagation(); removeFromTeam('task', ${t.id}, ${team.id})" title="Cofnij do puli">↻</button>
                            <button class="iconBtn" style="color:var(--danger);" onclick="event.stopPropagation(); deleteTask(${t.id})" title="Usuń zadanie na stałe">🗑</button>
                        </div>
                    </div>
                `).join('') : 
            ...
            // Umożliwienie przeciągania pracowników i zadań pomiędzy zespołami
            card.querySelectorAll('.team-member-item, .team-task-item').forEach(item => {
                item.addEventListener('dragstart', (e) => {
                    const type = item.classList.contains('team-member-item') ? 'worker' : 'task';
                    draggedItem = {
                        id: parseInt(item.dataset.id),
                        type,
                        fromTeamId: parseInt(item.dataset.teamId)
                    };
                    e.dataTransfer.setData('text/plain', JSON.stringify(draggedItem));
...
async function handleDrop(e) {
    ...
    // Odczytaj dane przeciąganego elementu z dataTransfer (bardziej niezawodne niż globalny stan)
    let item = null;
    try {
    ...
    // Fallback do starego mechanizmu, jeśli z jakiegoś powodu brak danych w dataTransfer
    if (!item && draggedItem) {
        item = draggedItem;
    }

    // 1. Walidacja: Czy ciągniemy właściwy typ do właściwej strefy (worker do worker, task do task)
    if (!item || item.type !== zone.dataset.type) {
    ...
    try {
        ...
        if (item.type === 'worker') {
            const fromTeamId = item.fromTeamId ? parseInt(item.fromTeamId) : null;

            // Jeśli przeciągamy w obrębie tego samego zespołu – nic nie rób
            if (fromTeamId && fromTeamId === teamId) {
                showToast("Pracownik już jest w tym zespole.");
                return;
            }

            // Jeżeli pracownik jest już w innym zespole – najpierw usuń z poprzedniego
            if (fromTeamId && fromTeamId !== teamId) {
                const removeRes = await fetch('api/team-members.php', {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        team_id: fromTeamId,
                ...
                if (!removeJson.success) {
                    showToast(removeJson.message || "Nie udało się przenieść pracownika.");
                    return;
                }
            }

            // Teraz przypisz do nowego zespołu
        ...
        // 3. Obsługa PRZYPISYWANIA ZADANIA
            ...
            // Jeśli upuszczamy w ten sam zespół – nic nie zmieniamy
            if (fromTeamId && fromTeamId === teamId) {
                showToast("Zadanie już jest w tym zespole.");
                return;
            }
...
async function addTeam() {
    ...
    const nRaw = prompt("Nazwa zespołu:");
    if(!nRaw) return;
    const n = nRaw.trim();
    if (!n) return;
    ...
    try {
        const res = await fetch('api/teams.php', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({project_id:currentProjectId, team_name:n})
        });
        const r = await res.json();
        if (r.success) {
            selectProject(currentProjectId);
            ...
            alert(r.message || 'Nie udało się dodać zespołu.');
        }
    } catch (e) {
        console.error(e);
        alert('Błąd połączenia przy dodawaniu zespołu.');
    }
}
...
async function addProject(nRaw) {
    if (!nRaw) return;
    const n = nRaw.trim();
    if (!n) return;
    ...
    // Lokalna walidacja: unikalna nazwa budowy w aktualnie załadowanych danych
    const existsLocal = (allProjects || []).some(p => (p.name || '').trim().toLowerCase() === n.toLowerCase());
    if (existsLocal) {
        alert('Budowa o tej nazwie już istnieje. Użyj innej nazwy.');
        return;
    }
    ...
    try {
        ...
        if (r.success) {
            ...
            alert(r.message || 'Nie udało się dodać budowy.');
        }
    } catch (e) {
        console.error(e);
        alert('Błąd połączenia przy dodawaniu budowy.');
    }
}
...
async function editProjectName() {
    ...
    const sel = document.getElementById('buildingSelect');
    const nRaw = prompt("Nowa nazwa:", sel.options[sel.selectedIndex].text);
    if (!nRaw) return;
    const n = nRaw.trim();
    if (!n) return;

    // Lokalna walidacja unikalności (ostateczna walidacja jest po stronie serwera)
    const existsLocal = (allProjects || []).some(p => p.id !== currentProjectId && (p.name || '').trim().toLowerCase() === n.toLowerCase());
    if (existsLocal) {
        alert('Inna budowa o tej nazwie już istnieje. Użyj innej nazwy.');
        return;
    }
    ...
    try {
        const res = await fetch('api/projects.php', { 
            method: 'PUT', 
            headers: {'Content-Type': 'application/json'}, 
            body: JSON.stringify({ id: currentProjectId, name: n }) 
        });
        const r = await res.json();
        if (r.success) {
            loadData();
            showToast('Zmieniono nazwę budowy');
        } else {
            alert(r.message || 'Nie udało się zmienić nazwy budowy.');
        }
    } catch (e) {
        console.error(e);
        alert('Błąd połączenia przy zmianie nazwy budowy.');
    }
}
...
// ==================== EDYCJA PRACOWNIKA W OKNIE MODALNYM ====================
function openWorkerEditDialog(workerId, currentName) {
    openTextModal(
        'Edytuj pracownika',
        'Imię i nazwisko',
        async (newNameRaw) => {
            ...
            try {
                const res = await fetch('api/workers.php', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: workerId, full_name: newName })
                });
                const r = await res.json();
                if (r.success) {
...
window.editWorkerInPlace = function(workerId, currentName) {
    openWorkerEditDialog(workerId, currentName);
};
...
function editWorkerName(id, oldName) {
    openWorkerEditDialog(id, oldName);
}
